...
Setup задаёт два вопроса: чем вы занимаетесь и какой AI-инструмент используете. Перед записью он показывает план и просит подтверждение.

## Несколько направлений или всё

Чтобы совместить несколько наборов, повторяйте `--profile`; добавьте `core` для базовых правил и специалистов. Например, для разработки и видео:
...
Чтобы установить все профили, используйте `all` — он уже включает `core`:
...
Интерактивный setup добавляет Foundation (`core`) автоматически. При явном выборе профилей добавьте `--profile core` сами, либо используйте `--profile all`. Точный состав профиля можно посмотреть до установки:
